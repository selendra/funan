...
export default function Send() {
  ...
  function handleConfirm() {
    if (!amount || !destination) return toast.error('Please fill the form!')
    if (!isvalidSubstrateAddress(destination))
      return toast.error('Look like wallet address is not correct!')
    if (balanceNotEnough(amount)) return toast.error('Not enough balance!')
    setModal(true)
  }
  ...
  function handleTransfer() {
    ...
    trx = api.tx.balances.transfer(destination, parsedAmount)
    return trx
  }
